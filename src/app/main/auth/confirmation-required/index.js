...
function ConfirmationRequiredPage() {
  ...
  const onSubmitCode = () => {
    ...
    if (token) {
      setLoading(true);
      AuthService
        .verifyAccount(token, verificationCode)
        .then((result) => {
          setLoading(false);
      ...
      <Paper className="w-full sm:w-auto min-h-full sm:min-h-auto rounded-0 py-32 px-16 sm:p-48 sm:rounded-2xl sm:shadow">
        <div className="w-full max-w-320 sm:w-320 mx-auto sm:mx-0">
          ...
          <Typography className="mt-16">
            An email with verification code has been sent to your email address. Please enter verification code below to confirm your email address and verify your account.
          </Typography>
          <div className="mt-24">
